...
Note that since Carbide Create is a cross-platform application (Mac OS and Windows) there are certain differences in the menu structures and keyboard shortcuts typical to such applications. Windows has a **Help** menu with About, Register, and Help options, while the Mac OS version as a Carbide Create menu with **About Carbide Create**, and a **Help** menu with Register, Help, and so forth. The Mac OS will use the command key for keyboard shortcuts where appropriate, while the Windows version will use the ctrl key.
